...
namespace directional
  ...
  bool inline bar_chains(const TriMesh& mesh,
                             ...
                             const double margin = 0.1)
  {
    ...
    VectorXd origA = mesh.faceAreas*2.0;
    ...
    for (int i=0;i<isoE.rows();i++){
      ...
      int v0e0=mesh.F(isoOrigE(i,0),(isoOrigE(i,1)+1)%3);
      int v1e0=mesh.F(isoOrigE(i,0),(isoOrigE(i,1)+2)%3);
      int v0e1=mesh.F(isoOrigE(i,0),(isoOrigE(i,2)+1)%3);
      int v1e1=mesh.F(isoOrigE(i,0),(isoOrigE(i,2)+2)%3);
      
      RowVector3d e0Vec = mesh.V.row(v1e0)-mesh.V.row(v0e0);
      RowVector3d e1Vec = mesh.V.row(v1e1)-mesh.V.row(v0e1);
      ...
      double baryCoords0 = (isoV.row(isoE(i,0))-mesh.V.row(v0e0)).norm()/e0Length;
      double baryCoords1 = (isoV.row(isoE(i,1))-mesh.V.row(v0e1)).norm()/e1Length;
      ...
      VBar<<mesh.V.row(v0e0).array() + (mesh.V.row(v1e0)-mesh.V.row(v0e0)).array()*baryBottom0,
              mesh.V.row(v0e0).array() + (mesh.V.row(v1e0)-mesh.V.row(v0e0)).array()*baryTop0,
              mesh.V.row(v0e1).array() + (mesh.V.row(v1e1)-mesh.V.row(v0e1)).array()*baryBottom1,
              mesh.V.row(v0e1).array() + (mesh.V.row(v1e1)-mesh.V.row(v0e1)).array()*baryTop1;
