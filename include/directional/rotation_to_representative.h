...
#ifndef ROTATION_TO_REPRESENTATIVE_H
#define ROTATION_TO_REPRESENTATIVE_H
...
{
	// Converts a rotation-angle representation ("trivial connection"), and a global rotation on face 0, to a representative that encodes
	// a unit-norm N-rosy that conforms to these rotation angles.
    // The rotation angles must respect the triviality constraints within the degree N or the results would be only leas-squares fitting to the input
    // Note: the rotation angles i rotate the directional from face EF(i,0) to face EF(i,1)
    // Also Note: the global rotation on face 0 is somewhat arbitrary. "0" means exactly "B1(f(0))".
    //
	// Inputs:
	...
	//  delta:          #E angles that encode the rotation angles (deviation from parallel transport)
	...
	//Version with only the raw mesh as input
	IGL_INLINE void rotation_to_representative(const Eigen::MatrixXd& V,
		const Eigen::MatrixXi& F,
		const Eigen::VectorXd& rotationAngles,
		const int N,
