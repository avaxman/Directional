...
void NFunctionMesher::segment_arrangement(const std::vector<Segment2>& segments,
    ...
    for (int i=0;i<segments.size();i++){
        ...
        for (ENumber intParam: data[i].intParams){
            ENumber t = (intParam - *data[i].intParams.begin())/(*data[i].intParams.rbegin()-*data[i].intParams.begin());
    ...
    for (int i=3;i<segments.size();i++) {   //starting from non-triangle segments
        for (int j = i + 1; j < segments.size(); j++) {
            ...
            Eigen::Matrix<ENumber, 2, 1> t1t2 = I2dts.block(data[i].origNFunctionIndex*2*linePencilSize+2*data[j].origNFunctionIndex, 0, 2, 2)*currI + t00s.segment(data[i].origNFunctionIndex*2*linePencilSize+2*data[j].origNFunctionIndex, 2);
            ...
            ENumber t1 = (t1t2(0) - *data[i].intParams.begin())/(*data[i].intParams.rbegin()-*data[i].intParams.begin());
            ENumber t2 = (t1t2(1) - *data[j].intParams.begin())/(*data[j].intParams.rbegin()-*data[j].intParams.begin());
