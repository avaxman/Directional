...
{    
		...
		double &error)
	{
		...
		VectorXd reducedAngles = reducedCycles.transpose()*solver.solve((-reducedcycleCurvature + cycleNewCurvature));
		...
		error = (reducedCycles*reducedAngles - (-reducedcycleCurvature + cycleNewCurvature)).lpNorm<Infinity>();
	}
	...
	{
		Eigen::SimplicialLDLT<Eigen::SparseMatrix<double> > solver;
		trivial_connection(V, F, basisCycles, indices, cycleCurvature, solver, N, adjustAngles, error);
	}
		...
		double &error)
	{
		...
		Eigen::VectorXd cycleCurvature;
		cycle_curvature(V, F, EV, EF, B1, B2, basisCycles, cycleCurvature); 
		trivial_connection(V, F, basisCycles, indices, cycleCurvature, N, adjustAngles, error);
	}
}
