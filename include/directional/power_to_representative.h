...
{
	// Converts a field in Cartesian complex representation ("u^N") and extracts a representative vector.
	// Input:
                                              ...
                                              Eigen::MatrixXd& U)
	{
		...
		U.conservativeResize(B1.rows(), 3);
		for (int f = 0; f < B1.rows(); ++f)
		{
			...
			U.row(f) = B1.row(f) * root.real() + B2.row(f) * root.imag();
		}
	}
