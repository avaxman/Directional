#ifndef DIRECTIONAL_EXTRINSIC_VERTEX_TANGENT_BUNDLE_H
#define DIRECTIONAL_EXTRINSIC_VERTEX_TANGENT_BUNDLE_H
...
Eigen::Matrix3d shortest_rotation_matrix(const Eigen::Vector3d& n1,
                                          const Eigen::Vector3d& n2)
{
    Eigen::Vector3d u = n1.normalized();
    Eigen::Vector3d v = n2.normalized();

    double c = u.dot(v);

    // Only handle the opposite case
    if (c < -0.999999999)
    {
        Eigen::Vector3d axis = u.unitOrthogonal();
        ...
        return Eigen::Matrix3d::Identity() + 2.0*K*K;
    }
...
class ExtrinsicVertexTangentBundle : public TangentBundle{
public:
    ...
    void inline init(const TriMesh& _mesh){
        ...
        //connection is the parallel transport of bases (inversely)
        ...
        for (int i = 0; i < mesh->EV.rows(); i++) {
            Eigen::Matrix3d shortR = shortest_rotation_matrix(mesh->vertexNormals.row(mesh->EV(i, 0)).transpose(), mesh->vertexNormals.row(mesh->EV(i, 1)).transpose());
            Eigen::RowVector3d transVBx = shortR*mesh->VBx.row(mesh->EV(i,0)).transpose();
            connection(i) = std::complex(transVBx.dot(mesh->VBx.row(mesh->EV(i,1))), transVBx.dot(mesh->VBy.row(mesh->EV(i,1))));
            connection(i) = connection(i)/std::abs(connection(i));
        }
        ...
        //currently using wrong geometric masses
