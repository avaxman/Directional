...
while (wconst<10e5)
    ...
    origValues = xcurr(fixedIndices);
    fixedValues = (round(fraction*xcurr(fixedIndices)-roundingOffset)+roundingOffset)/fraction;
    ...
    if (max(roundDiffs)>10e-7) || (size(C,1)~=0 && max(abs(C*xcurr))>10e-7)  %did not converge, try again with different integers
        ...
        %if (fraction==8)
            ...
            %killing all other integers
            %wconst=1;
            %fixedIndices=fixedIndices(end);
            %fixedValues=fixedValues(end);
            %leftIndices=setdiff(singularIndices,fixedIndices);
