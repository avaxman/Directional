...
namespace directional {
    ...
    // Polycurl reduction only works with face-based fields and tangent bundles
    // Input:
    ...
    //   originalField     The initial field to be optimized for curl-reduction
    // Output:
    ...
    // Inputs:
    //   cffsoldata                   PolyCurlReductionSolverData object that holds all intermediate
    //                                data needed by the solve routine, with their values at the current time instance.
    //   params                       solver parameters (see below)
    //   currentField                #F by 6 list of the representative vectors of the current frame field to be used as a starting point
    //   fieldNotCCW     boolean, determines whether the representative vectors in the current field are in
