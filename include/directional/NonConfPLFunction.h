...
#ifndef DIRECTIONAL_NON_CONF_PIECEWISE_LINEAR_FUNCTION_H
#define DIRECTIONAL_NON_CONF_PIECEWISE_LINEAR_FUNCTION_H
...
namespace directional{
    ...
    class NonConfPLFunction{
    public:

        const TriMesh* mesh;
        Eigen::Vector<NumberType, Eigen::Dynamic> midEdgeValues;

        NonConfPLFunction(){}
        ~NonConfPLFunction(){}

        void init(const TriMesh& _mesh, const Eigen::Vector<NumberType, Eigen::Dynamic> _midEdgeValues){
            mesh = &_mesh;
            midEdgeValues = _midEdgeValues;
        }

        //Only good for triangle meshes right now
        //TODO: this is not true atm
        NumberType value(const int faceIndex,
                         const Eigen::VectorXd& baryCoords){

            NumberType currValue = 0;
            for (int i=0;i<baryCoords.size();i++)
                currValue += baryCoords[i]*midEdgeValues(mesh->FE(faceIndex,i));  //incorrect!!
        ...
        Eigen::SparseMatrix<NumberType> gradient_matrix(){
            Eigen::SparseMatrix<NumberType> G(3*mesh->F.rows(), midEdgeValues.size());
            ...
            for (int i=0;i<mesh->F.rows();i++){
                ...
                for (int j=0;j<3;j++)
                    for (int k=0;k<3;k++)
                        GTris.push_back(Eigen::Triplet<NumberType>(3*i+k, mesh->FE(i,j), -ep((j+2)%3,k)/(faceArea)));  //TODO: check this is correct
        ...
        void gradient(directional::CartesianField& gradField){
            assert("NonConfPLFunction::gradient(): gradField is no of the correct type! " && gradField.N==1 && gradField.tv->discTangType()==discTangTypeEnum::FACE_SPACES);
            gradField.fieldType=fieldTypeEnum::RAW_FIELD;
            Eigen::SparseMatrix<NumberType> G = gradient_matrix();
            Eigen::Vector<NumberType, Eigen::Dynamic> fieldVector = G*midEdgeValues;
