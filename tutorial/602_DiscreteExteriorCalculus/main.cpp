...
int main()
{
    ...
    for (int k = 0; k < hodgeStar.outerSize(); ++k)
        for (Eigen::SparseMatrix<double>::InnerIterator it(hodgeStar, k); it; ++it)
            if (it.row() == it.col()) {
                if (it.value() <= 0) {
                    std::cerr << "Non-positive diagonal at index " << it.row() << ": " << it.value() << std::endl;
                }
            }
    ...
    Eigen::MatrixXd formField = viewer.set_1form(z1,"Coexact field", 0, 0, 2);
    Eigen::MatrixXd formFieldDiag = viewer.set_1form(z1Diag,"Coexact field diag", 0, 1, 2);
