...
{
    // Calculate the field
    directional::complex_field(V, F, cIDs, cValues, N, complex);
    
    // Convert it so it can be drawn
    directional::complex_to_representative(V, F, complex, N, representative);
    ...
    if (cIDs.rows()!=0){
        directional::principal_matching(V, F, representative, N, effort);
        
        directional::get_indices(V, F, cycles, effort, N, indices);
...
int main()
{
    ...
    // Load mesh
    //igl::readOBJ(TUTORIAL_SHARED_PATH "/rocker-arm2500.obj", meshV, meshF);
